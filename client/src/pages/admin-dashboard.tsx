...
export default function AdminDashboard() {
  ...
  const { data: stats } = useQuery({
    queryKey: ["/api/stats"],
    refetchInterval: 5000, // Refetch stats every 5 seconds for real-time updates
  });
        ...
        <div className="space-y-1 mb-3">
          ...
          <div className="flex justify-between items-center text-xs">
            <span className="text-muted-foreground">
              Aberto: {new Date(ticket.createdAt).toLocaleString('pt-BR')}
            </span>
            {ticket.status === 'open' ? (
              <RealTimeTimer
              ...
              />
            ) : ticket.status === 'in_progress' && ticket.acceptedAt ? (
              <RealTimeTimer
                startTime={ticket.acceptedAt}
          ...
          <div className="space-y-1 text-xs">
            {ticket.resolvedAt && (
              <div className="text-green-600">
                <span className="font-medium">Resolvido em:</span> {new Date(ticket.resolvedAt).toLocaleString('pt-BR')}
              </div>
            )}
            {ticket.acceptedAt && ticket.resolvedAt && (
              <div className="text-purple-600">
                ...
                {(() => {
                  const diffMs = new Date(ticket.resolvedAt).getTime() - new Date(ticket.acceptedAt).getTime();
            ...
            {ticket.acceptedAt && (
              <div className="text-blue-600">
                ...
                {(() => {
                  const diffMs = new Date(ticket.acceptedAt).getTime() - new Date(ticket.createdAt).getTime();
                  const hours = Math.floor(diffMs / (1000 * 60 * 60));
                  const minutes = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
                  return hours > 0 ? `${hours}h ${minutes}m` : `${minutes}m`;
                })()}
