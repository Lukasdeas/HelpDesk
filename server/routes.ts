...
import { insertTicketSchema, insertUserSchema, insertCommentSchema, tickets } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/tickets", async (req, res) => {
    try {
      ...
      // Definir status padrão como 'waiting' e prioridade como 'waiting'
  ...
  app.get("/api/files/:filename", async (req, res) => {
    try {
      ...
      if (!userRole || !['technician', 'admin'].includes(userRole as string)) {
  ...
  // Manter compatibilidade com sistema antigo (deprecated)
